...
public class FluidStackListWrapper implements IFluidHandler {
    ...
    public FluidStack drain(FluidStack resource, FluidAction action) {
        ...
        for (int i = 0; i < getTanks(); i++) {
            ...
            if (action.execute()) {
                FluidStack returned = existing.copy();
                returned.setAmount(existing.getAmount() - toDrain);
                return FluidStackHooksForge.toForge(tanks.set(i,FluidStackHooksForge.fromForge(returned)));
            } else {
