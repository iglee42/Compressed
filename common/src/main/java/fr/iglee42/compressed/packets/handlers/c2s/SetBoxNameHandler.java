...
public class SetBoxNameHandler {
    ...
    public void handle(final SetBoxNamePayload payload, final NetworkManager.PacketContext context) {
        context.queue(()->{
            ...
            if (box == null){
                player.displayClientMessage(Component.translatable("message.compressed.player_box_not_found").withStyle(ChatFormatting.RED),true);
                return;
            }

            box.setName(payload.name());
            data.setDirty();
            player.displayClientMessage(Component.empty().append(Compressed.PREFIX).append(Component.empty().append(Component.translatable("message.compressed.box_rename_success",payload.name().replace('&','§'))).withStyle(ChatFormatting.GREEN)),false);
        });
    }
